...
import zedit2.Audio.Companion.playSequence
import java.awt.*
...
class CodeEditor(
    ...
    init {
        ...
        val w = worldEditor.globalEditor.getInt("CODEEDITOR_WIDTH")
        val h = worldEditor.globalEditor.getInt("CODEEDITOR_HEIGHT")
    ...
    private fun propCol(property: String, def: String): Color {
        return Color(worldEditor.globalEditor.getIntRadix(property, def, 16))
    }
    ...
    val code: ByteArray
        get() {
            ...
            if (worldEditor.globalEditor.getBoolean("AUTO_INSERT_NEWLINE")) {
    ...
    private fun addCheckboxTo(centrePane: JPanel, s: String, cfg: String) {
        ...
        val chk = JCheckBox(s, ge.getBoolean(cfg, false))
        chk.addItemListener { e: ItemEvent? -> ge.setBoolean(cfg, chk.isSelected) }
    ...
    private fun generalSearch(document: String, caret: Int, text: String): ArrayList<Int>? {
        ...
        var text = text
        val regex = worldEditor.globalEditor.getBoolean("FIND_REGEX", false)
        val caseSensitive = worldEditor.globalEditor.getBoolean("FIND_MATCH_CASE", false)
