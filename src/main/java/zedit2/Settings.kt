...
import zedit2.BoardManager.Companion.generateBoardSelectArray
import zedit2.CP437.font
import zedit2.CP437.toBytes
import zedit2.CP437.toUnicode
import zedit2.ElementListModel.Companion.renderer
import zedit2.ElementListModel.Companion.transferHandler
import java.awt.*
...
class Settings(private val editor: WorldEditor) {
    ...
    private fun elementConfig(): Component {
        ...
        for (f in 3..10) {
            ...
            val elText = ge.getString(String.format("F%d_MENU", f), "")
        ...
        applyButton.addActionListener { e: ActionEvent? ->
            for (f in 3..10) {
                ...
                ge.setString(String.format("F%d_MENU_%d", f, menuModel.size), "")
            }
            editor.createMenu()
            editor.updateMenu()
            applyButton.isEnabled = false
    ...
    private fun testConfig(): Component {
        val ge = editor.globalEditor
        val testChangeBoard =
            JCheckBox("Start the tested world on the current board", ge.getBoolean("TEST_SWITCH_BOARD", false))
    ...
    private fun writeToConfig(cfgMap: HashMap<String, Any>) {
        ...
        for (cfgKey in cfgMap.keys) {
            ...
            if (cfgVal is Int) ge.setInt(cfgKey, (cfgVal as Int?)!!)
            else if (cfgVal is Boolean) ge.setBoolean(cfgKey, (cfgVal as Boolean?)!!)
            else if (cfgVal is String) ge.setString(cfgKey, (cfgVal as String?)!!)
    ...
    private fun colConfig(
        ...
        val col = Color(ge.getString(cfgString, "000000").toInt(16))
    ...
    private fun spinConfig(
        ...
        val label = JLabel(desc)
        label.border = BorderFactory.createEmptyBorder(0, 0, 0, 12)
        val spin = JSpinner(SpinnerNumberModel(ge.getInt(cfgString, 0), 1, 9999, 1))
    ...
    private fun chkConfig(
        ...
        val isSelected = ge.getBoolean(cfgString, false)
    ...
    private fun textConfig(
        ...
        val label = JLabel(desc)
        label.border = BorderFactory.createEmptyBorder(0, 0, 0, 12)
        val tf = JTextField(ge.getString(cfgString, ""))
    ...
    private fun dirConfig(
        ...
        val label = JLabel(desc)
        label.border = BorderFactory.createEmptyBorder(0, 0, 0, 12)
        val path = ge.getString(cfgString, "")
        val file = File(path)
        ...
        chdir.addActionListener { e: ActionEvent? ->
            ...
            if (dirOk) {
                ...
                fc.currentDirectory = ge.defaultDirectory
            }
    ...
    private fun injectConfig(
        ...
        val isSelected = ge.getBoolean(cfgString, false)
        val delay = ge.getInt(cfgString + "_DELAY", 0)
